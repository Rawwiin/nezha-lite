...
// 维护操作速率限制：最少间隔 10 分钟，防止频繁 VACUUM 导致数据库锁定
var (
	lastMaintenanceAt   time.Time
	lastMaintenanceMu   sync.Mutex
	maintenanceInterval = 10 * time.Minute
)

func runMaintenance(c *gin.Context) (any, error) {
	lastMaintenanceMu.Lock()
	if !lastMaintenanceAt.IsZero() && time.Since(lastMaintenanceAt) < maintenanceInterval {
		lastMaintenanceMu.Unlock()
		return nil, errors.New("maintenance rate limited: please wait at least 10 minutes between calls")
	}
