	...
	return []*model.Server{
		{
			...
			State:      &model.HostState{CPU: 0.1, TcpConnCount: 42, UdpConnCount: 7, ProcessCount: 128},
		...
		},
		{
			...
			State:      &model.HostState{CPU: 0.2, TcpConnCount: 99, UdpConnCount: 3, ProcessCount: 256},
		...
		},
		{
			...
			State:      &model.HostState{CPU: 0.3, TcpConnCount: 15, UdpConnCount: 2, ProcessCount: 64},
		...
		},
		{
			...
			State:      &model.HostState{CPU: 0.4, TcpConnCount: 77, UdpConnCount: 5, ProcessCount: 512},
...
func TestFilterServersForViewerGuestHidesPrivateAndRedactsHost(t *testing.T) {
	...
	if assert.NotNil(t, alicePublic) {
		...
		// State.Filter() 必须对匿名用户隐藏 TCP/UDP 连接数和进程数
		assert.Zero(t, alicePublic.State.TcpConnCount, "guest must not see TcpConnCount")
		assert.Zero(t, alicePublic.State.UdpConnCount, "guest must not see UdpConnCount")
		assert.Zero(t, alicePublic.State.ProcessCount, "guest must not see ProcessCount")
		assert.NotZero(t, alicePublic.State.CPU, "non-sensitive CPU metric stays visible")
	}
}
...
func TestFilterServersForViewerNonOwnerMemberMatchesGuest(t *testing.T) {
	...
	if assert.NotNil(t, bobPublic) {
		...
		assert.Zero(t, bobPublic.State.TcpConnCount, "non-owner State must be filtered")
		assert.Zero(t, bobPublic.State.ProcessCount, "non-owner State must be filtered")
	}
}
...
func TestFilterServersForViewerOwnerSeesOwnHiddenAndFullHost(t *testing.T) {
	...
	if assert.NotNil(t, aliceHidden) {
		...
		assert.NotZero(t, aliceHidden.State.TcpConnCount, "owner sees full State on own server")
		assert.NotZero(t, aliceHidden.State.ProcessCount, "owner sees full State on own server")
	}
	...
	if assert.NotNil(t, bobPublic) {
		...
		assert.Empty(t, bobPublic.Host.Version)
		assert.Zero(t, bobPublic.State.TcpConnCount, "non-owner State is still redacted")
	}
}
...
func TestFilterServersForViewerAdminSeesAllWithFullHost(t *testing.T) {
	...
	for _, s := range out {
		...
		assert.NotZero(t, s.State.TcpConnCount, "admin must see full State including TcpConnCount")
		assert.NotZero(t, s.State.ProcessCount, "admin must see full State including ProcessCount")
	}
}
