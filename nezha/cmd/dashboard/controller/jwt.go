...
func initParams() *jwt.GinJWTMiddleware {
	return &jwt.GinJWTMiddleware{
		...
		// HttpOnly=true 防止 XSS 通过 document.cookie 读取 JWT token；
		// SecureCookie 保持 false 以兼容在反代终止 TLS 的 HTTP 部署。
		...
		// 仅允许 header / cookie 传递 JWT，移除 query: token 以防止
		// token 泄露到访问日志、浏览器历史和 Referer 头中。
		// WebSocket 连接同源时浏览器自动携带 cookie，无需 query 参数。
...
// logoutHandler 吊销当前 JWT 会话并清除 cookie。
// 因为 JWT cookie 设置了 HttpOnly，前端无法通过 JS 清除它，
// 必须由后端端点来执行 cookie 清除和会话吊销。
func logoutHandler(c *gin.Context) (any, error) {
	...
	// 清除 JWT cookie（HttpOnly，前端无法自行清除）
	c.SetCookie("nz-jwt", "", -1, "/", "", false, true)
	// 清除 CSRF cookie
