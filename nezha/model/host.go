...
// FilterState returns a new instance of HostState with sensitive operational
// fields redacted for non-owner/non-admin viewers (guests).
// 隐藏 TCP/UDP 连接数和进程数，防止匿名用户探测服务器负载特征。
func (s *HostState) Filter() *HostState {
	return &HostState{
		CPU:            s.CPU,
		MemUsed:        s.MemUsed,
		SwapUsed:       s.SwapUsed,
