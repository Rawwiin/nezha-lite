...
// Provider 通过 Webhook 更新 DNS 记录的 provider
...
// SetRecords 通过 Webhook 发送 DNS 更新请求
func (provider *Provider) SetRecords(ctx context.Context, zone string,
	...
	for _, rec := range recs {
		...
		case libdns.Address:
			...
			// WebhookURL 是用户可控的（GHSA-6x26-5727-rrm9）；请求和客户端配对，
			// URL 验证和 DialContext 固定由单次 DNS 解析驱动。不要替换为 utils.HttpClient。
...
func (provider *Provider) prepareRequest(ctx context.Context) (*http.Request, *http.Client, error) {
	...
	// 单次 SSRF 检查 + dial 固定；返回的客户端必须被调用方使用，
	// 以保证 dialer 固定的 IP 和验证过的 URL 保持同步
...
// formatWebhookString 替换 webhook 模板中的占位符
